...
const ImageSelector = ({ imageId, imageName }) => {
  const [image, setImage] = useState(new ImageProp());
  if (imageId) {
    return (
      <>
        <img
          className="object-contain w-32 h-32 rounded-xl flex items-center justify-center"
          src={`${imageName}`}
          alt=""
        />
      </>

      // implement component that displays image in full screen when it is clicked
    );
  }
  return (
    <div className="bg-slate-100  w-32 h-32 rounded-xl flex items-center justify-center text-orange-500">
      <svg
        width="41"
        height="40"
        viewBox="0 0 41 40"
